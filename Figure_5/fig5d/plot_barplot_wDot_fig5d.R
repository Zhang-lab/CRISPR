...
data<- read.table("fig5d_data", sep="\t", row.names=1, header=T)
...
color_vec<- c("grey60", "rosybrown3","rosybrown3", "steelblue3", "steelblue3") 

pdf("fig5d.pdf");

p<- ggplot(t_data1, aes(x=factor(rownames(t_data1), levels=c("Ctrl_sgRNA", "PTPN14_sgRNA_1", "PTPN14_sgRNA_2", "TET2_sgRNA_1", "TET2_sgRNA_2")) , y=MEAN, fill=color_vec, ymin=mean_vec-se_vec, ymax=mean_vec+se_vec) )
p<- p+ geom_bar(stat="identity", fill=color_vec)
p<- p+ geom_errorbar(width=0.2, position=position_dodge(0.05))
p<- p+ labs(y="Relative cell count", x="")
