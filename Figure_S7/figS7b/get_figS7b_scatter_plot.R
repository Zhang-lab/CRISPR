...
data<- read.table("STB_mt_vs_STB_wt_DEG_full_list.csv", sep=",", header=T, row.names=1)

CPM_data<- read.table("CPM.csv", sep=",", header=T, row.names=1)

deg_data<- read.table("STB_mt_vs_STB_wt_DEG_significant_only.csv", sep=",", header=T, row.names=1)
...
gene_list<- c("cgb1", "cgb2", "e2f8", "ervfrd-1", "fosb", "jund", "slc13a4", "slc38a3", "trpv2", "dusp6", "nf2", "bmpr1a", "pdgfa")
...
p<- p+ ggtitle("STB KO vs WT") + labs(x="log2(mean CPM of KO and WT)", ylab="LFC");
...
p<- p+ geom_text_repel(max.overlaps=Inf, size=5, nudge_y=0.1, nudge_x=0.1, box.padding=1, xlim=c(NA, Inf), ylim=c(-4, Inf) )
...
pdf("figS7b_STB_KOvsWT_LFC_CPM_mean.pdf", width=10);
p;
dev.off();
