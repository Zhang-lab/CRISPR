...
#count_data<- read.table("raw_data_10x.txt", sep="\t", header=T, row.names=1, comment.char="")

mm_only_data<- read.table("jax_mm_only_genes", header=F)
shared_data<- read.table("jax_overlap_genes", header=F);

mm_only_genes<- toupper(mm_only_data[,1])
shared_genes<- toupper(shared_data[,1])

#Get alias
#shared_genes[grep("VIRMA", shared_genes)]<- "KIAA1429"
#mm_only_genes[grep("ARMH3", mm_only_genes)]<- "C10orf76"
#mm_only_genes[grep("SUPT3", mm_only_genes)]<- "SUPT3H"
mm_only_genes[which(mm_only_genes == "CCN1")]<- "CYR61"
mm_only_genes[which(mm_only_genes == "CERT1")]<- "COL4A3BP"
mm_only_genes[which(mm_only_genes == "FHIP1A")]<- "FAM160A1"
mm_only_genes[which(mm_only_genes == "H13")]<- "HM13"
mm_only_genes[which(mm_only_genes == "KIFBP")]<- "KIF1BP"
mm_only_genes[which(mm_only_genes == "TEX19.1")]<- "TEX19"
mm_only_genes[which(mm_only_genes == "ZFP568")]<- "ZNF568"


gene_vec0<- c(mm_only_genes, shared_genes) 
...
#    metadata_location<- metadata[metadata[,"location"]==locations[i],]
#    celltypes<- unique(metadata_location[,"annotation"])
...
#col_vec<- c(alpha(rainbow(3)[1], 0.5), rep("lightgrey", 2), alpha(rainbow(3)[2], 0.5), rep("lightgrey", 9), alpha(rainbow(3)[3], 0.5), rep("lightgrey", 4))


p1<- ggplot(log2cpm_data_mmHs_longer, aes(x=factor(Celltype, levels=celltype_order), y=log2cpm))
#p1<- ggplot(cpm_data_mmHs_longer, aes(x=factor(Celltype, levels=celltype_order), y=cpm))
...
p1<- p1+ theme(legend.position="none", plot.title=element_text(hjust=0.5, size=25, face="bold"), axis.text=element_text(size=14), axis.title=element_text(size=20) )



pdf("jax_boxplot_across_celltype_combineTissue_mergeCelltype_mmHs_polish.pdf", width=8, height=7);
...
dev.off()
    ...
    hs_mm_shared_genes<- shared_genes;
    p_list<- vector('list', ncol(log2cpm_data))
    p_count<- 0;
...
print(paste0("ymax=", ymax))
         ...
         p<- p+ labs(y=expression(paste(Log[2], "(CPM+1)")), x="Gene group",title=paste0(celltypes[k]," cell num=",n_cell))
    ...
    mat_vec<- rep(0, 3*ceiling(p_count /3) )
    mat_vec[1:p_count]<- 1:p_count;

    mat<- matrix(mat_vec, ncol=3, byrow=T)

    pdf(paste0("violin_EMTAB78_01_jax_mergeCelltype.pdf"), width=18, height=25);
    print(multiplot(plotlist=p_list, layout=mat) );
    dev.off();
