...
data1<- data.frame(cbind(1:nrow(data),as.character(data[,"Name",]),-log10(as.numeric(as.character(data[,"q.value.FDR.B.H"]))), as.character(data[,"direction"])) ) 

colnames(data1)<- c("pos", "name", "log10Q", "direction");
data1$direction<- factor(data1$direction, levels=c("Up", "Dn"));
...
p<- ggplot(data1, aes(x=pos, y=log10Q, fill=direction) ) + geom_bar(stat="identity")
p<- p+ scale_fill_manual(values=c(alpha("brown",0.3), alpha("steelblue", 0.3)) )

p<- p+ coord_flip()

p<- p+ geom_text(aes(label=name), y=0.01, hjust=0, size=5.4)
