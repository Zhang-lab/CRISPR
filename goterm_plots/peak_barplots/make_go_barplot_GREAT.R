...
tag<- "CnT_overlap_atac"
data<- read.table(tag, header=T, sep="\t")

data1<- data.frame(cbind(1:nrow(data),as.character(data[,"atac"]),as.character(data[,"Name",]),-log10(as.numeric(as.character(data[,"q.value.FDR.B.H"])))) ) 

colnames(data1)<- c("pos", "atac", "name", "log10Q");
...
p<- ggplot(data1, aes(x=pos, y=log10Q, fill=atac) ) + geom_bar(stat="identity"  )

p<- p+ coord_flip()

p<- p+ geom_text(aes(label=name), y=0.01, hjust=0, size=6)

p<- p+ scale_fill_manual(values=c(alpha("steelblue",0.3), alpha("#FFB900", 0.3)) )
...
pdf_filename<- paste0("barplot_",tag, ".pdf");
